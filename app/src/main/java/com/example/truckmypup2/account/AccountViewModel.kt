...
import com.example.truckmypup2.register.RegisterFormState
...
class AccountViewModel : ViewModel() {
    ...
    fun fetchAccountData(img: ImageView) {
    ...
    fun deleteAccount(callback: IHomeActivity){
        val userID = FirebaseAuth.getInstance().uid
        val fileRef = FirebaseStorage.getInstance().getReferenceFromUrl(_imageUrl.value!!)
        fileRef.delete().addOnSuccessListener {
            FirebaseFirestore.getInstance().collection("users").where(Filter.equalTo("id", userID))
                .get().addOnSuccessListener { docs ->
                    for(doc in docs)
                    {
    ...
    fun saveData(firstnameText: String, lastnameText: String, emailText: String, phoneText: String, passwordText: String ){
